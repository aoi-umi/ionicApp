...
export class ApiProvider {
    ...
    threadListGet(islandCode: string, id, page) {
        ...
        return this.get(this.common.stringFormat(config.GetThreadAPI, config.Host, id, page));
    }

    replyListGet(islandCode: string, id, page) {
        let config = IslandConfig[islandCode];
        return this.get(this.common.stringFormat(config.GetReplyAPI, config.Host, id, page));
    }
}